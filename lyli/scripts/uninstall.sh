...
PREFIX="${LYLI_UNINSTALL_PREFIX:-$HOME}"
APP_LABEL="${LYLI_UNINSTALL_APP_LABEL:-com.chambersxdu.lyli}"
CONFIG_DIR="$PREFIX/.config/lyli"
APP_LOG_FILE="$PREFIX/Library/Logs/lyli-app.log"
...
unregister_login_item() {
  local app_bin="/Applications/Lyli.app/Contents/MacOS/lyli"
  [[ "$PREFIX" == "$HOME" && -x "$app_bin" ]] || return 0
  if "$app_bin" --unregister-login-item >/dev/null 2>&1; then
    echo "  ✅ 已注销系统登录项(开机启动)"
  else
    echo "  ⚠️ 注销登录项失败,请到「系统设置 → 通用 → 登录项」手动移除 Lyli"
  fi
}
...
echo
echo "App 本体请自行拖进废纸篓: /Applications/Lyli.app"
