...
// QQ Music uses a DES variant. The embedded implementation is adapted from
// valenbine/LRC-GET (MIT); see THIRD_PARTY_NOTICES.md.
enum QQMusicDES {
    static func decrypt(_ hex: String) -> Data? {
        guard !hex.isEmpty, hex.count.isMultiple(of: 16),
              let context = JSContext() else { return nil }
        context.evaluateScript(script)
        guard context.exception == nil,
              let decrypted = context.objectForKeyedSubscript("decryptQrcHex")?
                .call(withArguments: [hex]).toString(),
              decrypted.count == hex.count else { return nil }
        var bytes = Data(capacity: decrypted.count / 2)
        let chars = Array(decrypted)
...
const qrcKey = Uint8Array.from("!@#)(*$%123ZXC!@!@#)(NHL", ch => ch.charCodeAt(0))
function decryptQrcHex(hex) {
  ...
  tripleDESKeySetup(qrcKey, schedule, DECRYPT)
  const output = new Uint8Array(input.length)
  for (let offset = 0; offset < input.length; offset += 8) {
    tripleDESCrypt(input.subarray(offset, offset + 8), output.subarray(offset, offset + 8), schedule)
  }
