...
private struct LyricsSettingsTab: View {
    ...
    private var sourcesCard: some View {
        SettingsCard {
            SettingsCardHeader(title: "歌词来源") { testAllSourcesButton }
            CardDivider()
        ...
        .alert("要把之前锁定的歌一并解锁吗？", isPresented: $showManualPickUnlockConfirm) {
            ...
            Button("一并解锁") {
                Task {
                    ...
                    showManualPickLockNotice(String(format: "已解锁 %@ 首", "\(n)"))
                }
            }
        } message: {
            Text(String(
    ...
    private var testAllSourcesButton: some View {
        Button {
            ...
            HStack(spacing: 4) {
                ...
                Text(isTestingLyricSources ? "测试中…" : "测试")
            }
        ...
        SettingsCard {
            ...
            SettingsRow(
                ...
                Picker("", selection: Binding(
                    ...
                    Text("不转换").tag(ChineseVariant.off)
                    Text("简体").tag(ChineseVariant.simplified)
                    Text("繁体").tag(ChineseVariant.traditional)
                }
                .pickerStyle(.segmented)
                .fixedSize()
            }
            ...
            SettingsRow(
                ...
                HStack(spacing: 8) {
                    Text("\(AppSettings.signedSeconds(ms: scopedOffsetMs))\("秒")")
                        .monospacedDigit()
...
private struct AboutSettingsTab: View {
    ...
    var body: some View {
        SettingsPage(title: "关于", heroImage: appIcon) {
            SettingsCard {
                SettingsRow(icon: "info.circle", title: LyliIdentity.displayName) {
                    Text(String(format: "版本 %@", versionString))
                        .foregroundStyle(.secondary)
                }
                CardDivider()
            ...
            SettingsCard {
                SettingsCardHeader(title: "许可与版权")
                CardDivider()
                SettingsRow(icon: "doc.text", title: "版权说明") {
                    Button("打开") { LegalNotices.openUsageNotice() }
                }
                CardDivider()
                SettingsRow(icon: "checkmark.seal", title: "第三方许可") {
                    Button("打开") { LegalNotices.openThirdPartyLicenses() }
                }
                CardDivider()
                SettingsRow(icon: "scroll", title: "开源许可证") {
                    Button("打开") { LegalNotices.openLicense() }
                }
            }
