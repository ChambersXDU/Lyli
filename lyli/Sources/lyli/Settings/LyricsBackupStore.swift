...
enum LyricsBackupStore {
    private static let logger = Logger(subsystem: "com.chambersxdu.lyli", category: "lyrics-backup")

    private static let enrichCacheURL = LyliPaths.configFile("lyli-enrich-cache.json")

    private static let enrichRestoreURL = LyliPaths.configFile("lyli-enrich-restore.json")
    ...
    static var autoSnapshotDir: URL {
        LyliPaths.configFile("lyrics-backups")
    }
    ...
    static func writeAutoSnapshot(reason: String) async -> URL? {
        ...
        let url = dir.appendingPathComponent("auto-\(reason)-\(formatter.string(from: Date())).lylibak")
        do {
    ...
    static func autoSnapshots() -> [Snapshot] {
        ...
        return urls
            .filter { $0.pathExtension == "lylibak" }
