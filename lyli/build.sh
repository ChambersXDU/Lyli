...
DEV_SIGN_NAME="Lyli Dev Signing"
SIGN_ID="${LYLI_SIGN_ID:-}"
if [ -z "$SIGN_ID" ]; then
...
<dict>
    ...
    <string>Lyli needs to send Apple Events to Apple Music to read the currently playing track and show synced lyrics.</string>
</dict>
</plist>
PLIST
...
if [ -n "$OLD_PIDS" ] && [ "$pid" = "$OLD_PIDS" ]; then
  ...
  echo "!! 想确认是不是这个原因:/usr/bin/log show --last 5m --predicate 'process == \"lyli\"' | grep 'blocked by'" >&2
  exit 1
fi
